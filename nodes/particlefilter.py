...
class ParticleFilter(object):
    ...
    def run(self):
        ...
        while not self.odom_received:
            pass
        self.current_pose = copy.deepcopy(self.recent_pose)
        self.prev_pose = copy.deepcopy(self.current_pose)

        #Initialize particles around guessed starting location
        self.init_particles(self.num_particles, self.current_pose)
        ...
        while not rospy.is_shutdown():
            ...
            if self.scan_received:
                ...
                self.current_pose = copy.deepcopy(self.recent_pose)
                self.current_scan = copy.deepcopy(self.recent_scan)
    ...
    def resample_particles(self):
        ...
        for p in range(0, len(self.p_set)):
            ...
            if self.p_set[p].weight > self.p_set[max_particle].weight:
                max_particle = copy.deepcopy(p)
        print(self.p_set[max_particle].weight)
        if (ParticleFilter.iteration % 1) == 0:
        ...
        for i in xrange(0, random_particles):
            ...
            newP =  copy.deepcopy(self.p_set[r])
            new_particles.append(newP)
